  ...
  useEffect(() => {
    if (user && matches.length > 0) {
      ...
      const filtered = matches.filter(match => {
        ...
        const canScore = (user.role === UserRole.ADMIN || user.role === UserRole.SCORER); 
        
        const scoringWindowOpen = matchTime - (60 * 60 * 1000); // Scoring opens 60 mins before
        const scoringWindowClose = matchTime + (4 * 60 * 60 * 1000); // And closes 4 hours after (example) for non-admin
